...
- **«Читайте также»** строится единым `/api/recommendations`: при включённом типе «Статьи» ручной порядок `related_article_ids` у статей/новостей имеет приоритет, затем идёт детерминированный смешанный fallback по связям/тегам/популярности. Глобальные настройки лежат в `site_settings._id=recommendations` и доступны admin на `/admin/related-content`: лимит, типы результатов и типы страниц показа. По умолчанию участвуют статьи, люди, команды КВН/шоу, шоу, города, KVN-страницы и квизы; новости доступны, но выключены. Скрывается только `archived`; текущая страница, дубли и битые ID исключаются.
