...
- **Импорт со старого сайта** — документы должны быть такими же, как сохранённые админкой (данные в полях документа, системные модули — только настройки вида, ссылки — абсолютные адреса нового сайта). Люди: `scripts/import_people_modx.py` читает SQL-дамп MODX (`backups/idemsku8_modx2.sql`) и сохраняет через `create_person`/`update_person`; все 1039 людей перенесены. Шоу — `scripts/import_shows_modx.py`, весь раздел перенесён, команды шоу — ещё нет (DATA_MODEL.md «Шоу»): сезоны и подпроекты — дочерние шоу `/shows/<шоу>/<сезон>`. Формат и соответствие полей — DATA_MODEL.md «Человек».
