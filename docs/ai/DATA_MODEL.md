...
`/people/{slug}`, `/kvn/teams/{slug}`, `/kvn/vl-kvn/vl-2015`, `/city/{slug}`, `/shows/…`. Ссылка может указывать на страницу,
которой ещё нет, — это нормально. Для разделов без новой структуры (ИГРА, Импровизация. Команды, `proekty/…`) хранится
