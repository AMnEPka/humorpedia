...
При включённом типе `article` сначала возвращает статьи из `related_article_ids` в ручном порядке, затем смешанный
детерминированный fallback по прямым связям, общим тегам, featured, рейтингу, просмотрам, дате, порядку типов и `_id`.
