...
При включённом типе `article` сначала возвращает статьи из `related_article_ids` в ручном порядке. Автоматическая часть
исключает URL, уже встречающиеся в данных страницы (для команд также в составах, проектах и турнирах), случайно выбирает
связанные страницы по прямым связям/общим тегам и резервирует одну карточку под находку из остального пула.
Псевдослучайный порядок стабилен в течение московских суток.
Типы результата: `article`, `person`, `kvn_team`, `show_team`, `show`, `city`, `kvn`, `quiz`, `news`; карточка содержит
`type`, `type_label`, нормализованный `cover_image` и готовый публичный `url`. Текущая страница, дубли, архивные и
отсутствующие документы исключаются; `draft` участвует наравне с `published`.

- `GET /settings` 🛡 admin и `PUT /settings` 🛡 admin — единые настройки `enabled`, `max_items` (1–12), редактируемый
  `result_types` и `apply_to` для статей, новостей, людей, команд КВН/шоу, шоу, городов и KVN-страниц. Ответы настроек
  имеют `Cache-Control: private, no-store`.
...
Публичные методы отдают участие для страницы человека и сгруппированные индивидуальные проекты участников команды КВН. В выдаче человека `show_url` ведёт на проект, а `link_url` — на проект для индивидуальной записи и на команду для командной; командный агрегат отбрасывает записи с `team_id`. В существующие карточки участников шоу проверенные ссылки добавляет `content_shows.py`. Проверка, ручная привязка и создание одного черновика доступны редактору. Подробно — [SHOW_APPEARANCES.md](SHOW_APPEARANCES.md).
