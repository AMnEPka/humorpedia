...
3. **Людей в БД почти нет** (10), хотя составы (3754 записи) и роли жюри (495 имён) уже ссылаются на них по имени/slug старого сайта. В дампе MODX `backups/idemsku8_modx2.sql` — 1039 опубликованных людей; импорт партиями на утверждение — `scripts/import_people_modx.py` (DATA_MODEL.md). После заведения связи проставятся автоматически. Три человека, заведённых раньше, — в старом формате (см. DATA_MODEL.md «Человек»).
3c. Ссылки в HTML уже импортированных команд (4893 шт.) — относительные старые (`people/x.html`): на новом сайте они разрешаются относительно текущего пути и ведут в никуда. Исправляется повторным импортом через `LinkMapper` (как у людей).
3d. На старом сайте 181 ссылка со страниц людей ведёт на отсутствующие страницы (`kvn/lampa/` и др.), несколько картинок — на кэш phpThumb (`assets/components/phpthumbof/cache/...`), которого нет в `images/`; 9 фото людей нет ни в одной папке картинок.
