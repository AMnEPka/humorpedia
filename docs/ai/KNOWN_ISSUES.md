...
3. ~~Результаты сезонов вшиты в `kvn.season_data`~~ — с этапа 1 есть модель `seasons`/`participations` (COMPETITIONS.md), но страницы команд всё ещё показывают таблицу игр из модуля, собираемого `refresh` (этап 2). Составов команд как данных нет: только текст в `text_block` «Состав команды» у 435 команд, людей в БД 3.
