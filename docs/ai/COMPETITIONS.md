...
- Если редактор удаляет ошибочную выбранную страницу человека, запись получает `person_link_disabled=true`: имя остаётся
  в составе без ссылки и больше не привязывается автоматически по совпадению имени. Выбор страницы вручную снимает запрет.
