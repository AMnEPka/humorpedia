...
```jsonc
{ "_id", "team_id": "teams._id", "person_id": "people._id|null", "matched_by": "slug|name|manual|null",
...
- **Связь с людьми.** Людей в БД почти нет, поэтому запись хранит имя и slug старого сайта. При создании/правке человека
  `link_person` проставляет `person_id` по slug (включая `old_urls` вида `/people/<slug>.html`) или однозначному имени;
  тёзки по имени не связываются. Удаление человека сбрасывает `person_id`.
...
| Роли жюри/ведущих | `person_id` в participations ставится при сохранении сезона; до пересборки сезона карьера человека находит роли по однозначному `name_key` |
...
| GET | `/people/{id_or_slug}/career` | 🔓 | команды человека + сезоны этих команд в годы его участия + роли в турнирах |
...
- `TeamRoster.jsx` — структурированный состав; `PersonCareer.jsx` — «Команды» и «В турнирах» на странице человека;
...
- Админка: вкладка «Состав» в редактировании команды (`TeamMembershipsEditor.jsx`).
...
## Дальше
- Страницы людей: в БД 3 человека, в `migration/people_list.json` — 1044 человека со старого сайта (slug, имя, фото).
