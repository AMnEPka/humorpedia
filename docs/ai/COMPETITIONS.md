...
- **Связь с людьми.** Явный slug (включая `old_urls` вида `/people/<slug>.html`) подтверждается автоматически и записывается
  в `person_id`. Совпадение только по однозначному имени или конфликт slug сохраняются кандидатом в `candidate_person_id`,
  но публичной ссылки не создают до подтверждения редактором. Удаление человека сбрасывает `person_id`.
