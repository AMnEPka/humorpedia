...
Бэкапы MongoDB создаются отдельным лёгким контейнером (Debian + `mongodump`). Без проверки по хешу — каждый запуск создаёт новый дамп и архив. Восстановление по-прежнему выполняется из контейнера backend.

> Сервис `backup` подключён **только в `docker-compose-cloud.yml`** (прод). В dev-конфигурации `docker-compose.yml` его нет —
> там бэкап запускается вручную (см. «Запуск образа backup без compose»).

## Компоненты

### 1. Контейнер бэкапа (`backup/`)

- **Образ**: Debian + MongoDB Database Tools (только mongodump).
- **Скрипт**: `backup/backup.sh` — делает `mongodump`, упаковывает в `humorpedia_backup_YYYYMMDD_HHMMSS.tar.gz`, удаляет старые (оставляет последние `KEEP_LAST_N`, в проде 14).
- **Автозапуск (прод)**: контейнер стартует вместе с `docker compose -f docker-compose-cloud.yml up`, делает бэкап при старте, затем раз в `BACKUP_INTERVAL` секунд (в проде по умолчанию 86400 — раз в сутки). Архивы пишутся в volume `backups_data`, который backend видит как `/app/backups`.
...
В проде контейнер `backup` уже крутится в фоне и бэкапит по расписанию. Чтобы сделать **одноразовый** бэкап и выйти (без фонового цикла):

```bash
docker compose -f docker-compose-cloud.yml run --rm -e BACKUP_INTERVAL=0 backup
```

Бэкап появится в volume `backups_data`.
